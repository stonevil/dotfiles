...
-- Parameters
-- default user option values do not touch, change them in osc.conf
local user_opts = {
	showwindowed = true,
	showfullscreen = true,
	idlescreen = true,
...
function render_elements(master_ass)
	-- when the slider is dragged or hovered and we have a target chapter name then we use it instead of the normal title. we calculate it before the render iterations because the title may be rendered before the slider.
...
function window_controls(topbar)
	...
	-- Window control buttons use symbols in the custom mpv osd font because the official unicode codepoints are sufficiently exotic that a system might lack an installed font with them, and libass will complain that they are not present in the default font, even if another font with them is available.
...
function render()
	...
	if state.active_element then
		-- mouse is held down on some element - keep ticking and ignore initReq till it's released, or else the mouse-up (click) will misbehave or get ignored. that's because osc_init() recreates the osc elements, but mouse handling depends on the elements staying unmodified between mouse-down and mouse-up (using the index active_element).
		request_tick()
...
-- duration is observed for the sole purpose of updating chapter markers positions. live streams with chapters are very rare, and the update is also expensive (with request_init), so it's only observed when we have chapters and the user didn't disable the livemarkers option (update_duration_watch).
...
mp.observe_property("osd-dimensions", "native", function(name, val)
	-- (we could use the value instead of re-querying it all the time, but then we might have to worry about property update ordering)
	request_init_resize()
end)
...
-- mode can be auto/always/never/cycle the modes only affect internal variables and not stored on its own.
function visibility_mode(mode, no_osd)
	...
	-- Reset the input state on a mode change. The input state will be recalculated on the next render cycle, except in 'never' mode where it will just stay disabled.
	mp.disable_key_bindings("input")
	mp.disable_key_bindings("window-controls")
